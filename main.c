...
void jLine( struct Vec2 *a, struct Vec2 *b, guchar msk )
{
   ...
   p -= 700 * 4; if ( p[3] < 252 ) { int res = p[0] + 87; p[0] = ( res > 255 ) ? 255 : res;

       res = p[1] + 87; p[1] = ( res > 255 ) ? 255 : res;

       res = p[2] + 87; p[2] = ( res > 255 ) ? 255 : res; p[3] = 252; }

   p -= 700 * 4; if ( p[3] < 252 ) { int res = p[0] + 63; p[0] = ( res > 255 ) ? 255 : res;

       res = p[1] + 63; p[1] = ( res > 255 ) ? 255 : res;

       res = p[2] + 63; p[2] = ( res > 255 ) ? 255 : res;  p[3] = 252; }

   p += 700 * 4 + 700 * 4 + 700 * 4; if ( p[3] < 252 ) {

       int res = p[0] + 87; p[0] = ( res > 255 ) ? 255 : res;
   ...
   p -= 1 * 4; if ( p[3] < 252 ) { int res = p[0] + 87; p[0] = ( res > 255 ) ? 255 : res;

       res = p[1] + 87; p[1] = ( res > 255 ) ? 255 : res;

       res = p[2] + 87; p[2] = ( res > 255 ) ? 255 : res;

       p[3] = 252; }

   p -= 1 * 4; if ( p[3] < 252 ) {  int res = p[0] + 63; p[0] = ( res > 255 ) ? 255 : res;

       res = p[1] + 63; p[1] = ( res > 255 ) ? 255 : res;

       res = p[2] + 63; p[2] = ( res > 255 ) ? 255 : res; p[3] = 252; }

   p += 3 * 4; if ( p[3] < 252 ) {  int res = p[0] += 87; p[0] = ( res > 255 ) ? 255 : res;
